...
├── ui.py                    # Live session window (transcript + robot face)
├── robot_face.py            # Vector-drawn animated robot face (Tkinter Canvas)
...
Running `python main.py` opens a window with two parts:
- **Left: an animated robot face** — reacts live to every gesture/emotion
  the LLM triggers (waves on greeting, smiles + rosy cheeks on `celebrate`,
  sleepy eyes on `think`, etc.), blinks idly when nothing else is happening,
  and animates its mouth while Lily is actually speaking.
- **Right: a live transcript** — what Whisper heard, what Lily said, and
  the currently detected student emotion, so you can follow the session
  without relying on audio alone during dev/debug.

- `python main.py --child aanya` → voice mode **with the UI** (default)
...
The face is driven by `robot_interface.py`'s UI hook — every time the LLM
calls `robot_gesture` or `robot_speak_emotion`, the face updates immediately,
independent of your physical robot hardware. This means the on-screen face
works today even before `HARDWARE_CONNECTED` is wired to your actual robot.
